...
#define SOURCE_IP "192.168.82.134"
...
int main() {
    ...
    //recv
    int recv_len = -1;
    socklen_t temp = sizeof(dest_in);
    if ((recv_len = recvfrom(sock, packet, sizeof(packet) - 1, 0, (struct sockaddr *) &dest_in, &temp)) == -1) {
        printf("recvfrom() failed with error code : %d", errno);
    }

    inet_ntop(AF_INET, &dest_in.sin_addr, packet, sizeof(packet));

    //print details of the client/peer and the data received
    printf("ping reply from %s:%d, ", packet, ntohs(dest_in.sin_port));
    printf(" %d bytes,  ", recv_len);
    printf("Data: %s,  ", packet + IP4_HDRLEN + ICMP_HDRLEN);

    //timer ends (end-start = rtt)
    gettimeofday(&end, NULL);
    long micro = (end.tv_sec - start.tv_sec) * 1000000 + end.tv_usec - start.tv_usec;
    double milli = (double) micro/1000;
    printf("with RTT: %ld micro s' (%.6g milli s')\n", micro, milli);
